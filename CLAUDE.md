...
- **Analytics:** Google Analytics 4, same property as the main Wix site (Google tag `G-F4XCCCJTWW`, inline in `index.html`). The tag only arms on `map.lifeinlongboatkey.com` — local dev and the redirecting Firebase hostnames define `gtag()` but never load the library, so they can't pollute the property. Because `www.` and `map.` share the root domain, the GA cookie is shared and cross-site visits count as one session. Embed iframes fire page_views too (hostname `map.…` + `?embed=` in the URL distinguishes them in reports). On top of the base tag, a custom event layer (`src/assets/js/modules/analytics.js` + one-line `track()` calls at the interaction choke points) instruments the in-app journey: filter usage, zero-result dead-ends, community views with source attribution, listing/community-page click-throughs, and mobile drawer/view events — every event tagged with `app_surface` (full map vs. each embed variant) and the live `result_count`. Event reference + required GA4 console setup (custom dimensions, key events, funnel) in `docs/analytics-events.md`. Interaction events only — nothing fires on load, and map pans/zooms and hovers are deliberately untracked (noise).
...
- **Campaign deep links** — email/social buttons can pre-filter the full map via URL params: `?community=<slug>` (opens that community focused with its details panel; on phones it lands on the map flown to the community, pin highlighted, since the mobile panel is a fullscreen overlay; if active filters would hide it — e.g. the default 'Currently for sale' toggle and no active listings — it still renders as a temporary highlighted map-only exception (`state.focusException`) that lapses when attention moves to another community — the Wix dynamic pages' "See It On The Map" button target; slug = last segment of the community's `pageUrl`, Velo snippet in `docs/wix-embed-snippet.md`), `?type=condos|neighborhoods` (singular forms accepted; pre-selects the matching Community Type pill — `condos` also seeds the Condominiums Home Type checkbox, uncheckable like any user selection), `?area=north|mid|south` (aliases `north-end`/`mid-key`/`south-end`; checks the Location filter and lands framed on the zone's pins), `?amenity=<slug>[,slug]` (slugified amenity names, e.g. `beach-club-access`), and `?view=map|list` (arrival view — lets `?group=bay-isles&view=map` land on the map instead of the group's list default). On the full map, `?group=` is a **clearable filter**, not a scope: it seeds `state.group` and renders a dismissible "Showing: Bay Isles" chip at the top of the filter rail — the ✕ or Clear All reveals the whole island. (Group *embeds* keep hard working-set scoping; the Bay Isles page's embed stays Bay Isles.) Deep links land on the map unless `?view=list`, and on a fresh navigation they override saved session filters (back/forward returns still restore them, including a cleared group). Param parsing lives in `embed.js` (`getDeepLinkParams`), seeding in `main.js` `bootFull()`.
